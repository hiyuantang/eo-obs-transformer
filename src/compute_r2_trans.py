...
def compute_r2(model: nn.Module, dataloader: DataLoader, device: torch.device) -> float: #
    ...
    with torch.no_grad():
        ...
        for images, labels, masks in pbar:
            ...
            masks_float = masks.to(device, dtype=torch.float32)
    ...
    valid_indices = [idx for idx, (p, l) in enumerate(zip(all_preds, all_labels)) if np.isfinite(p) and np.isfinite(l)]
    if len(valid_indices) < len(all_labels):
        print(f"Warning: Filtered out {len(all_labels) - len(valid_indices)} NaN/Inf values.")
        all_labels = [all_labels[i] for i in valid_indices]
        all_preds = [all_preds[i] for i in valid_indices]

    if not all_labels or not all_preds:
        print("Warning: No finite predictions/labels remaining after filtering.")
        return 0.0
...
def main(model_path: str):
    ...
    print("Initializing model architecture (ResNetTransformer)...") 
    model = PyTorchResNetTransformer.get_model_with_head(MODEL_PARAMS) 
    ...
    try:
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            print("Checkpoint dictionary loaded successfully (extracted 'model_state_dict').")
        elif isinstance(checkpoint, dict) and 'epoch' in checkpoint:
             print("Warning: Checkpoint dictionary found, but key 'model_state_dict' missing.")
             print("Keys found:", checkpoint.keys())
             print("Attempting to load dictionary directly, but this might fail.")
             model.load_state_dict(checkpoint, strict=False) 
             print("Attempted loading with strict=False due to missing 'model_state_dict' key.")
        else:
            model.load_state_dict(checkpoint)
            print("Model weights loaded successfully (assumed direct state_dict).")

    except KeyError as ke:
         print(f"Error: Key missing when loading state_dict. Probable cause: incorrect key name in checkpoint file. Error: {ke}")
         if 'checkpoint' in locals() and isinstance(checkpoint, dict):
             print("Checkpoint keys found:", checkpoint.keys()) 
         exit()
    except Exception as e:
        print(f"Error loading model state_dict: {e}")
        print("\nIf the error persists after fixing the loading method, double-check:")
        print("1. The 'model_state_dict' key exists in the .pth file (check keys printed above if applicable).")
        print("2. The MODEL_PARAMS defined in this script perfectly match the architecture used during training for this specific checkpoint.")
        exit()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a trained ResNet-Transformer model.")
