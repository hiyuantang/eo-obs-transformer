...
def collate_fn(batch: List[Dict]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    # Initialize tensors based on actual_batch_size and fixed N_FRAMES
    ...
    for i, item in enumerate(valid_batch_items):
        ...
        # Fill image tensor for the 10 fixed frames
...
if __name__ == "__main__":
    ...
    with open(log_file_path, 'a', newline='') as f:
        ...
        if write_header:
            log_writer.writerow(['Epoch', 'TrainLoss', 'ValLoss']) 
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (images, labels, masks) in enumerate(train_pbar):
            ...
            # --- Calculate Final Loss for Backward ---
        ...
        with torch.no_grad():
            for images, labels, masks in val_pbar:
                ...
                # --- Calculate Aggregated Loss for Reporting ---
                total_loss_batch = masked_per_element_loss.sum()
                num_unmasked = masks.sum() 
        ...
        if avg_val_loss < best_val_loss:
            ...
            model_save_name = f"{MODEL_PARAMS.get('name', 'resnet_transformer')}_best.pth" 
    ...
    best_model_path = os.path.join(LOG_DIR, f"{MODEL_PARAMS.get('name', 'resnet_transformer')}_best.pth")
